...
export default function Orders({ state, updateState }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newOrder: CustomerOrder = {
      id: generateId(),
      orderNumber: generateOrderNumber('ORD', state.orders.length),
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border mb-6">
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              <select required value={form.customerId} onChange={e => setForm({...form, customerId: e.target.value})} className="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-emerald-500 outline-none">
            ...
            <div className="mb-4">
              ...
              {items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 mb-2 items-center">
                  <input placeholder="Description" value={item.description} onChange={e => updateItem(idx, 'description', e.target.value)} className="col-span-5 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none" />
                  <input type="number" min="1" placeholder="Qty" value={item.quantity} onChange={e => updateItem(idx, 'quantity', parseInt(e.target.value) || 0)} className="col-span-2 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none" />
                  <input type="number" min="0" placeholder="Rate" value={item.unitPrice} onChange={e => updateItem(idx, 'unitPrice', parseFloat(e.target.value) || 0)} className="col-span-2 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none" />
          ...
          <div className="bg-white rounded-xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-4 flex gap-2">
              <span className={`px-3 py-1 rounded-full text-sm ${
                ...
                selectedOrder.status === 'partial' ? 'bg-yellow-100 text-yellow-700' :
          ...
          <table className="w-full">
            ...
            <tbody>
              {state.orders.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-8 text-gray-400">No orders yet. Create your first order!</td></tr>
