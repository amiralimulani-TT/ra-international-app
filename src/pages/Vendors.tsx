...
export default function Vendors({ state, updateState }: Props) {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl p-6 shadow-sm border mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input required placeholder="Vendor Name *" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-emerald-500 outline-none" />
      ...
      <div className="mb-4">
        <input placeholder="Search vendors..." value={search} onChange={e => setSearch(e.target.value)} className="w-full sm:w-64 border rounded-lg px-4 py-2 focus:ring-2 focus:ring-emerald-500 outline-none" />
      </div>
